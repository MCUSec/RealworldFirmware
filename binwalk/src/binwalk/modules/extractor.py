...
class Extractor(Module):
    ...
    CLI = [
        ...
        Option(short='cl',
               long='clean',
               kwargs={'remove_original': True},
               description='Delete the original file after extraction')
    ]
    ...
    KWARGS = [
        ...
        Kwarg(name='remove_original', default=False)
    ]
    ...
    def callback(self, r):
        ...
        if r.valid and r.extract and r.display and (not self.max_count or self.extraction_count < self.max_count):
            ...
            binwalk.core.common.debug("Extractor callback for %s @%d [%s]" % (r.file.name,
                                                                              r.offset,
                                                                              r.description))
            # print("Extractor callback for %s(%s) @%d [%s]" % (r.file.name,
            #                                                   r.name,
            #                                                   r.offset,
            #                                                   r.description))
    ...
    def remove_original_extracted(self, tf=None):
        if tf is not None:
            self.remove_original = tf

        return self.remove_original
    ...
    def extract(self, offset, description, file_name, size, name=None):
        ...
        if offset > 0:
            ...
            binwalk.core.common.debug("Offset is greater than 0 '%s'" % description)
            return (None, None, False, str(None))
        
        if file_name.endswith(".apk"):
            print(f"We do not extract apks ({file_name})")
            binwalk.core.common.debug("We do not extract apks: '%s'" % file_name)
            return (None, None, False, str(None))
                    ...
                    if self.remove_after_execute and (extract_ok == True or i == (len(rules) - 1)):
                        ...
                        try:
                            if file_md5(fname) == fname_md5:
                                os.unlink(fname)
                            #Remove original if flag is set
                            #Remove original if custom output directory is set
                            #remove original if custom directory is NOT path of original
                            #print(f"Directory: {self.directory}")
                            if self.remove_original and self.base_directory and self.directory in file_name:
