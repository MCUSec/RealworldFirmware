...
def print_proto_message(message, indent=""):
        ...
        if descriptor.label == descriptor.LABEL_REPEATED:
            for item in field:
                if descriptor.message_type:
                    print(f"{indent}{descriptor.name} {{")
                    print_proto_message(item, indent + "  ")
                    print(f"{indent}}}")
                else:
                    print(f"{indent}{descriptor.name}: {item}")
        else:
            if descriptor.message_type:
                print(f"{indent}{descriptor.name} {{")
                print_proto_message(field, indent + "  ")
                print(f"{indent}}}")
            else:
                print(f"{indent}{descriptor.name}: {field}")


# Each plugin must be subclassed from binwalk.core.plugin.Plugin
class NordicDat(binwalk.core.plugin.Plugin):
    '''
    A binwalk plugin to analyze the data file of nordic zip 
    '''

    MODULES = ['Signature']
    dat_message = None
    def init(self):
        self.dat_message = Packet()
        return 
    def pre_scan(self):
        return 
    def new_file(self, fp):
        # TODO: add other judgement for nordic dat 
        if fp.path[-4:] == '.dat':
            with open(fp.path, 'rb') as file:
                read_byte = file.read()
            self.dat_message.ParseFromString(read_byte)
        return 
    def scan(self, result):
        return 
    def post_scan(self):
        if self.dat_message:
            print_proto_message(self.dat_message)
