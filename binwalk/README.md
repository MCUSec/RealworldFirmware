...
Command to recursively extract files and clean up intermediate files
```
binwalk -e -M --rm -cl -C "/path/to/step1_bins" <compressed_file_name>
```
-cl/--clean Must be used with "-e" and "-C/--directory=", it removes the original extracted files in th eoutput directory, allowing us to only keep the binary/firmware image. 
