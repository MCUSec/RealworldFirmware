        ...
        for (long ab : AbsAddrSet) {
            ...
            for (long start : entry) {
                ...
                if (candidate < 0 || candidate > Constant.MAX_BASE)
                    continue;
                if (candidate % 2 != 0)
                    System.out.println("const2 gives odd candidate");
        ...
        for (Address add: straddrs) {
            ...
            for (long ab: AbsAddrSet) {
                ...
                if (candidate < 0 || candidate > Constant.MAX_BASE)
                    continue;
                if (candidate % 2 != 0)
                    System.out.println("const3 gives odd candidate");
                addScore(scores, candidate);
            }
        }
