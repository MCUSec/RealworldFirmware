...
public class Main {
    ...
    public static void main(String[] args) throws
        ...
        try {
            Path path = Paths.get(Constant.DIRECTORY_NAME);
            Files.createDirectory(path);
        } catch(Exception e) {
            //System.out.println("/tmp/firmxaytest/ directory already exist");
        }
        ...
        if (base == -1) {
            ...
            base = -1;
            System.out.println("Base: 0x" + base);
        } else {
    ...
    private static void getBase(String programName, String tag, String projectName) throws
            IOException, VersionException, CancelledException, DuplicateNameException, InvalidNameException {
        // Not retrieve the base from file because it causes problems with files with same the name in different folders
        ...
        //}
        long base = BaseAddressSolver.getBaseAddressWithConstraint(programName, projectName);
        Fileutil.writeToFile("./base/base.txt", String.format("%s\t%d", tag, base), false);
    }
