...
public class FloatSimulation implements SimulationObjects {
    ...
    public HashSet<?> handleInvokeStmt(InvokeStmt stmt, String signature, InvokeExpr expr, HashMap<Value, HashSet<?>> currentValues) {
        // :
        if (signature.equals("<java.lang.StringBuilder: java.lang.StringBuilder append(float)>") ){
            return transferValuesAndAppend(stmt, ((VirtualInvokeExpr) expr).getBase(), SimulationUtil.getStringContentFromFloat(expr.getArg(0), currentValues), currentValues);
        }
        return null;
    }
